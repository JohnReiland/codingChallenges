...
A and B such that every edge in the graph connects a node in set A and a node in
set B.
...
Researching the word bipartite has taught me that if a graph contains an odd
cycle, it cannot be bipartite, and if it doesn't, it must be bipartite.
Playing around with inputs has revealed that it can be revealed whether a graph
contains an odd cycle by attempting to sort its nodes under the assumption that
the graph is bipartate. Succeeding or failing in that task will reveal the graph
